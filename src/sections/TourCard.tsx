...
import { getReadableDestination, getDepartureDateBadgeLabel, buildTitleSummary, hasReliableField } from '@/lib/tour-display';
...
export const TourCard = memo(function TourCard({
  ...
  const hasReliableGroupSize = hasReliableField(tour, 'groupSize');
  const hasReliableDiscount = hasReliableField(tour, 'discountRate');
  const hasReliableOriginalPrice = hasReliableField(tour, 'originalPrice');
  const hasReliableLeisureLevel = hasReliableField(tour, 'leisureLevel');
  ...
  return (
    ...
    >
      <div className="relative h-44 overflow-hidden bg-stone-100 sm:h-52">
        ...
        {hasReliableDiscount && tour.discountRate && tour.discountRate > 0 && (
      ...
      <CardContent className="p-4 sm:p-5">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          ...
          {hasReliableLeisureLevel && tour.leisureLevel === 'medium' && (
            <Badge
              variant="secondary"
          ...
          {hasReliableLeisureLevel && tour.leisureLevel === 'hard' && (
            <Badge
              variant="secondary"
        ...
        <div className="mt-4 space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-stone-500">
            ...
            {hasReliableGroupSize && tour.groupSize && <span className="flex items-center gap-1.5"><Users className="h-3.5 w-3.5" />{tour.groupSize}</span>}
          </div>
        </div>
        ...
        <div className="mt-4 flex items-end justify-between gap-3 border-t border-stone-100 pt-4 sm:mt-5">
          <div>
            ...
            <div className="mt-1 flex items-baseline gap-1.5">
              ...
              {hasReliableOriginalPrice && tour.originalPrice && tour.originalPrice > tour.price && (
