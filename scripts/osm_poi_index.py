...
DEFAULT_OUTPUT_PATH = Path(__file__).resolve().parent.parent / "public" / "data" / "osm-poi-index.json"
HOTEL_TOURISM_VALUES = {"hotel", "guest_house", "hostel", "motel", "resort", "chalet", "apartment"}
TOURISM_VALUES = HOTEL_TOURISM_VALUES | {"attraction", "theme_park", "zoo", "museum", "viewpoint", "camp_site"}
NAME_KEYS = ("name", "name:zh", "alt_name", "official_name", "brand", "operator", "name:en")
...
def poi_kind(tags: dict) -> str:
    ...
    if tourism in HOTEL_TOURISM_VALUES or str(tags.get("amenity") or "") in {"hotel", "guest_house"}:
        return "hotel"
...
def address_from_tags(tags: dict, region: dict) -> dict:
    address = {
        "country": str(tags.get("addr:country") or region.get("country") or "中国").strip(),
        "province": str(tags.get("addr:province") or region.get("province") or "").strip(),
        "city": str(tags.get("addr:city") or "").strip(),
        "district": str(tags.get("addr:district") or "").strip(),
        "locality": str(tags.get("addr:subdistrict") or tags.get("addr:place") or "").strip(),
...
def make_poi(*, osm_type: str, osm_id: int, tags: dict, latitude: float, longitude: float, region: dict) -> dict | None:
    names = split_names(tags)
    kind = poi_kind(tags)
    if not names or not kind or not (-90 <= latitude <= 90 and -180 <= longitude <= 180):
        return None
    address = address_from_tags(tags, region)
    formatted_parts = [names[0], address.get("street", ""), address.get("locality", ""), address.get("district", ""), address.get("city", ""), address.get("province", ""), address.get("country", "")]
...
def build_index(rows: list[dict], regions: list[str]) -> dict:
    unique = {row["osmId"]: row for row in rows if isinstance(row, dict) and row.get("osmId")}
    pois = sorted(unique.values(), key=lambda row: (row["normalizedName"], row["osmId"]))
    return {
        "schemaVersion": 1,
...
def download(url: str, destination: Path, max_bytes: int) -> None:
    request = Request(url, headers={"User-Agent": "EverywhereWeGoGz-OSM-Indexer/1.0"})
    with urlopen(request, timeout=120) as response, destination.open("wb") as handle:
        content_length = int(response.headers.get("Content-Length") or 0)
        if content_length and content_length > max_bytes:
            raise ValueError(f"{url} is larger than the configured download limit")
        written = 0
        while chunk := response.read(1024 * 1024):
            written += len(chunk)
            if written > max_bytes:
                raise ValueError(f"{url} exceeded the configured download limit")
            handle.write(chunk)
...
def extract_pois(pbf_path: Path, region: dict) -> list[dict]:
    try:
        ...
        raise RuntimeError("Install pyosmium (pip install osmium) before building the OSM POI index") from error
...
def load_regions(path: Path, selected: list[str]) -> list[dict]:
    ...
    wanted = set(selected)
    selected_regions = [region for region in regions if isinstance(region, dict) and (not wanted or region.get("id") in wanted)]
...
def main() -> None:
    ...
    parser.add_argument("--region", action="append", default=[], help="region id from osm-poi-regions.json; repeatable")
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT_PATH)
    parser.add_argument("--work-dir", type=Path, help="temporary download directory; omitted means an auto-cleaned directory")
