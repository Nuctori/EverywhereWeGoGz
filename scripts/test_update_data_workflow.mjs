...
const availabilityWorkflow = fs
.readFileSync(availabilityWorkflowPath, "utf8")
.replace(/\r\n/g, "\n");
...
mustInclude(
'- cron: "0 3 * * 1"',
"expected scheduled update-data workflow to run weekly on Monday UTC",
);
mustInclude(
"skip_crawls:",
...
mustInclude(
"DETAIL_FETCH_MODE: geo",
"expected weekly refresh to enrich details only for tours without destination coordinates",
);
...
"expected update-data workflow to serialize data writes",
);
assert.ok(
...
mustInclude(
`- name: ${stepName}`,
`expected workflow step ${stepName} to exist`,
);
const stepStart = workflow.indexOf(`- name: ${stepName}`);
const stepChunk = workflow.slice(stepStart, stepStart + 300);
assert.ok(
/timeout-minutes:\s+\d+/.test(stepChunk),
...
assert.ok(
!workflow
.slice(auditStepStart, auditStepStart + 200)
.includes("continue-on-error: true"),
"expected audit merged data to remain a blocking gate",
);

const fallbackJobStart = workflow.indexOf(
...
assert.ok(
workflow
.slice(gzlStepStart, gzlStepStart + 300)
.includes("timeout-minutes: 180"),
...
const stepChunk = workflow.slice(stepStart, stepStart + 300);
assert.ok(
stepChunk.includes("timeout-minutes: 60"),
`expected ${stepName} to keep a 60 minute timeout`,
);
}
...
mustInclude(
`- name: ${stepName}`,
`expected update job step ${stepName} to exist`,
);
}
...
assert.ok(
gitAddMatch,
"expected Check if data changed to stage generated data files",
);
...
assert.ok(
preflightStart > -1 && firstCrawlStart > preflightStart,
"expected preflight job before crawl jobs",
);
...
assert.ok(
preflightJob.includes(snippet),
`expected preflight job to include ${snippet}`,
);
}
...
assert.ok(
start > -1 && end > start,
`expected ${jobName} job block to be found`,
);
const job = workflow.slice(start, end);
assert.ok(
job.includes("needs: preflight"),
`expected ${jobName} to depend on preflight`,
);
}
...
assert.ok(
updateJobStart > -1,
"expected update-and-deploy job block to be found",
);
const updateJob = workflow.slice(updateJobStart);
assert.ok(
updateJob.includes("needs.preflight.result == 'success'"),
"expected update job to require preflight success",
);

const jrtJobStart = workflow.indexOf("crawl-jrt365:");
const nextJobStart = workflow.indexOf("crawl-saihuitong:");
assert.ok(
jrtJobStart > -1 && nextJobStart > jrtJobStart,
"expected crawl-jrt365 job block to be found",
);
...
assert.ok(
jrtJob.indexOf("- name: Verify JRT365 crawl guard") <
jrtJob.indexOf("- name: Crawl JRT365 full"),
